...
! pb in units cells m-3, pbp in cells m-3 s-1
       pbp = CALC_PROKARYOTE_PRODUCTION(mu0, theta, f_T, ldoclimit_p, flimit_p, pb)
       !WRITE(*,*) 'pbp', pbp
...
! ligP because the export in P units
...
ELSE
   ...
   dltdt = dltdt + (ligP * rCP - lambda * lt - lt/ldoc * rCLig_in * pbp * cellsm32molm3 * 1.0_wp/pge) + &
                                                                      ligphi * pbl * kappa * cellsm32molm3
ENDIF
       ! Could also add a production term by prokaryotes here (pi term)
       ! rather unconstrained and complicates the model
       ! but can be added if interested in additional dynamics
...
!=======================================================================
! Calculate prokaryotic biomass production
       FUNCTION CALC_PROKARYOTE_PRODUCTION(mu0, theta, f_T, ldoclimit_p, flimit_p, pb)
       USE MOD_BOXES
       ...
       CALC_PROKARYOTE_PRODUCTION = mu0 * (f_T**theta) * pb * minval( &
